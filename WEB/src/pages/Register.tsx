    ...
    <div className="flex min-h-screen items-center justify-center px-4">
      <div className="w-full max-w-md space-y-6 rounded-2xl border border-(--border) bg-(--card) p-8">
        ...
        <form onSubmit={handleRegister} className="space-y-3">
          <input type="text" required value={username} onChange={(e) => setUsername(e.target.value)} placeholder="Имя пользователя" className="w-full rounded-lg border border-(--input) bg-(--background) px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-(--ring)/30" />
        ...
        <p className="text-center text-sm text-(--muted-foreground)">
          Уже есть аккаунт?
          <Link to="/login" className="font-medium text-(--primary) ml-1 hover:underline cursor-pointer">
            Войти
          </Link>
        </p>
      </div>
